...
class Mapping:
    ...
    def update_map(self, grid_map, pose, scan):
        ...
        """
        i = -1
        for dis in scan.ranges:
            i += 1
            angle = scan.angle_min + scan.angle_increment * i

            if dis <= scan.range_min or dis >= scan.range_max:
                continue

            x_las = dis * math.cos(angle + robot_yaw)
            y_las = dis * math.sin(angle + robot_yaw)

            x_map = pose.pose.position.x + x_las - origin.position.x
            y_map = pose.pose.position.y + y_las - origin.position.y


            map_x_id = int(x_map / resolution)
            map_y_id = int(y_map / resolution)

            self.add_to_map(grid_map, map_x_id, map_y_id, self.occupied_space)
