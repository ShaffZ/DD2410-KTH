...
class StateMachine(object):
    ...
    def check_states(self):
        ...
        while not rospy.is_shutdown() and self.state != 5 and self.state != 6:
            ...
            if self.state == 0:
                ...
                failed_tucking = self.play_motion_ac.wait_for_result(rospy.Duration(10.0))
                PoseStamped
                if failed_tucking:
                    ...
                    rospy.loginfo("%s: tuck arm", self.play_motion_ac)
                    self.state = 1
            ...
            if self.state == 1:
                try:
                    ...
                    cube_pose_msg.header.frame_id = self.robot_base_frame_nm
                    ...
                    if pick_req.success == True:
                        rospy.loginfo("%s: pick up the cube", pick_req)
                        self.state = 2
                    else:
                        rospy.loginfo("%s: failed to pick up the cube", pick_req)
                        self.state = 1
                except rospy.ServiceException as exc:
                    rospy.logerr("%s: pick up failed", self.node_name)
                    self.state = 5

            # Move to second table state
            if self.state == 2:
                move_msg = Twist()
            ...
            if self.state == 3:
                try:
                    ...
                    if place_req.success == True:
                        rospy.loginfo("%s: place the cube", pick_req)
                        self.state = 6
                    else:
                        rospy.loginfo("%s: failed to place the cube", pick_req)
                        self.state = 1
            ...
            if self.state == 6:
                rospy.logerr("%s: The state machine is over!", self.node_name)
